...
approximation is not in the linear finite-difference regime for sub-grid steps.
"""
import copy
...
def assert_symbolic_matches_numerical_jacobian(fit_object, params, *, rtol, atol, label):
    """Compare symbolic Jacobian against adaptive finite-difference numerical Jacobian.

    Ported from ~/git_repos/FLinst/tests/test_plot_current_rounding_helpers.py.
    Uses an adaptive step-size search to stay in the linear regime.
    """

    def adaptive_directional_derivative(baseline_vector, baseline_scale, base_value, shifted_vector):
        ...
        for _ in range(20):
            ...
            if max(np.linalg.norm(d) for d in deltas) < 1e3 * np.finfo(float).eps * baseline_scale:
                step *= 8.0
                continue
    ...
    for name in fit_object.parameter_names:
        ...
        try:
            numerical_columns.append(
                adaptive_directional_derivative(baseline, baseline_scale, base_value, shifted_vector)
            )
...
def test_jacobian_exponential():
    ...
    fit = _build_fit(t_arr, "t", expr, dict(A=dict(value=2.0), R=dict(value=1.5)))
...
# dt0 column — uses finite_difference_heaviside_derivative (a discrete DiracDelta
#              approximation); sub-grid shifts produce no residual change at all,
#              so the adaptive helper cannot find a linear regime.  We instead
#              check structural properties: exactly one nonzero entry, at the
#              correct grid location, with the correct sign.
# ---------------------------------------------------------------------------
...
def test_jacobian_heaviside_dt0_spike_location():
    ...
    assert spike_t == nearest_t, (
        f"spike at t={spike_t}, expected nearest grid point to t0={t0_val} which is t={nearest_t}"
    )
