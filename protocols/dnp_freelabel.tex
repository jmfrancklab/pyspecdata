\subsection{10\uM w/ 5 step $T_1$ vs. $p$ --repeat}\maxtime{3}\label{sec:4ohtseriesrepfresh_10uMt1vprep}
...
for index,file in enumerate(files):
   ...
   if centerfield != None:
      legendtext[index] += ', %0.03f $G$'%(
              centerfield)
   tempstr = r', SNR %0.2g $\int\int$ %0.3g'
   legendtext[index] += tempstr%(
           snr,integral[newname,-1].data[-1])
#xtl = ax.get_xticklabels()
#at.xaxis.tick_top()
#map( (lambda x: x.set_visible(False)), xtl)
plot(data.getaxis(newname)[mask],zeros(
    shape(data.getaxis(newname)[mask])),
    'k',alpha=0.2,linewidth=10)
fl.next('epr')
...
\fn{hydroxytempo_10uM_120123}

Overwrite exp 1 with desired parameters.
\fn{hydroxytempo_10uM_120122}
...
up to 5 $dB$.)}
\begin{lstlisting}
calcfielddata(9.789958,'oxotempo','cnsi')
\end{lstlisting}
...
Or, if necessary, run {\tt jf\_t10} with default parameters (should be min 1,max 2.5,steps 5,min ratio 0.08 (should have been 0.05 to match), max ratio 4, pull from exp 4, put in exp 101).
