...
with figlist_var() as fl:
    ...
    for filenum, (thisfile, thislabel) in enumerate(
        ...
        peaklist = d_abs.contiguous(lambda x: abs(x) > abs(x).data.max() / 2)[
            :3, :
        ]
        ...
        polybaseline = (
            ...
            .set_axis(fieldaxis, lambda x: x - middle_field)
            .eval_poly(c, fieldaxis)
        )
        polybaseline.set_axis(fieldaxis, lambda x: x + middle_field)
        ...
        fl.plot(
            ...
            alpha=0.5,
            label=f"{thislabel} (denominator ${signal_denom:~0.2L}$)",
        )
        fl.next("dblint")
        fl.plot(d_abs, alpha=0.5, label=f"{thislabel}")
        myconcs.append((d_abs / signal_denom.magnitude).data[-100:].mean())
        fl.next("dblint, normed\n→ check for ⅓")
        fl.plot(d_abs / d_abs.data[-100:].mean(), alpha=0.5)
        if filenum == 0:
    ...
    for j in range(len(myconcs)):
        print(
            "Bruker: %04.2f Ours: %04.2f"
            % (QESR_concs[j] / 1e-6, myconcs[j] / m_hardcode / 1e-6)
        )
