...
with figlist_var() as fl:
    ...
    for thislabel, thisscan in list_of_runs.items():
        if thislabel in [background,"Water"]:
            plot(dataWKBSA[thisscan], label=thislabel+'\n(no background subtraction)',
                    alpha=0.5)
        else:
            plot(dataWKBSA[thisscan]-bg_data, label=thislabel,
                    alpha=0.5)
    # }}}
    ...
    bg_data = dataWKBSA[list_of_runs[background]]
    all_data = concat([dataWKBSA[list_of_runs[k]]-bg_data for k in conc_labels],'concentration').setaxis('concentration',conc_values)
    # }}}
