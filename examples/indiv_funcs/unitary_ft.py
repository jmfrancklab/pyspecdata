...
logger.info(strm(
    "vector norm of fake data, before ft",
    linalg.norm(fake_data.data)
    ))
plot(fake_data, ax=ax_time, alpha=0.5, label="vector norm=%g"%linalg.norm(fake_data.data))
ax_time.set_title("time domain")
ax_freq.set_title("frequency domain")
fake_data.ft("t2", shift=True, unitary=True)
logger.info(strm(
    "vector norm of fake data, after unitary ft",
    linalg.norm(fake_data.data)
    ))
assert fake_data.get_ft_prop("t2", "dt") == 1e-3
plot(fake_data, ax=ax_freq, alpha=0.5, label="vector norm=%g"%linalg.norm(fake_data.data))
fake_data.ift("t2", unitary=True)
logger.info(strm(
    "vector norm of fake data, after unitary ift",
    linalg.norm(fake_data.data)
    ))
