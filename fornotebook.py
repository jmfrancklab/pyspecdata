...
def thisjobname():
	...
	fp.close()
	return retval
def lplot(fname,width=3,figure=False,dpi=200,grid=True,alsosave=None):
	'''
...
def save_variable(variable,content,disp=True):
	...
	return data
#{{{ Emax fitting
def emax_w_err_prop(path,
		name,
		expno,
		dBm,
		dBm_error = 0.2,
		showimage = True,
		integration_width = 0.15e3,
		show_errorbars = False,
		plot_string = 'o',
		filter_direct = False):
	r'''show plots for the Emax, including error propagation
actual Emax analysis still needs to be added,
for which the most recent version was done for the last group meeting
generates fig 1 + 2 from integrate Emax + fig 3 for emax curve w/ errorbars + fig 4 for straight line plot w/ errorbars'''
	scaling = 1e-3 # for dBm
	power = scaling*(10**(0.1*dBm))
	dBm_error = 0.2
	power_error = scaling*log(10)*(10**(0.1*dBm))*0.1*dBm_error
	# this should be pretty portable -- maybe stick in fornotebook.py
	integral,noiselevel = integrate_emax(path+name+'/',expno,showimage=showimage,usephase=True,integration_width=integration_width,usebaseline=True,plotcheckbaseline=False,center_peak=True,return_noise = True,filter_direct = filter_direct)
	# pulled figure 1 + 2 plotting from here + isolated from function
	figure(3) # put the errorbar plot in figure 3!
	normalizer_index = where(dBm==dBm.min())[0]
	normalizer = integral[normalizer_index].copy()
	normalizer_std = noiselevel[normalizer_index].copy()
	if len(normalizer)>1:
	   normalizer = normalizer[0]
	   normalizer_std = normalizer_std[0]
	order = argsort(-dBm)
	try:
		expno = expno[order]
		...
		raise CustomError('Everything\'s not sized right!: expno:',len(expno),'integral:',len(integral),' noiselevel:',len(noiselevel),' power:',len(power))
	power_error = power_error[order]
	noise_div = sqrt(
			(integral/(normalizer**2)*normalizer_std)**2
			+ (noiselevel/normalizer)**2)# variance of uncorr A/B =  (A/B^2*Bstd)^2+(Astd/B)^2
	if show_errorbars:
		errorbar(power/1e-3,integral/normalizer,xerr = power_error,yerr = noise_div)
	else:
		plot(power/1e-3,integral/normalizer,plot_string)
	ax = gca()
	yaxis = integral/normalizer
	ax.set_ylim([yaxis.min(),yaxis.max()])
	axis('tight')
	xlabel(r'power / $mW$')
	ylabel(r'$E$')
	figure(4)
	Einv = 1./(1.-integral/normalizer)
	Einv_error = 1./((1.-integral/normalizer)**2)*noise_div
	pinv = 1./power
	pinv_error = 1./(power**2)*power_error
	if show_errorbars:
		errorbar(pinv,Einv,xerr = pinv_error,yerr = Einv_error)
	else:
		plot(pinv,Einv,plot_string)
	ax = gca()
	ax.set_ylim([pinv.min(),pinv.max()])
	ylabel(r'$\frac{1}{1-E}$')
	xlabel(r'$\frac{1}{p}$ / $W^{-1}$')
