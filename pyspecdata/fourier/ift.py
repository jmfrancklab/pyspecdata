    ...
    **post-IFT**, we assume that the data has previously been FT'd
    ...
    for j in axes:
        if self.get_ft_prop(j) == False:
            ...
            raise ValueError(
                errmsg
                + "-- you can't IFT again unless you explicitly .set_ft_prop('"
                + str(j)
                + "',None), which is probably not what you want to do"
            )
    ...
    for j in range(0, len(axes)):
        ...
        if p2_post_discrepancy is not None:
            ...
            try:
                ...
                if self.data.dtype != "complex128":
                    raise TypeError(
                        "You tried to ift nddata that was of type "
                        + str(self.data.dtype)
                    )
                else:
                    raise TypeError(e)
        ...
        self.data = np.fft.ifft(self.data, axis=thisaxis)
