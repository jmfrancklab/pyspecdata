    ...
    **post-FT**, we assume that the data has previously been IFT'd
    ...
    if self.data.dtype == np.float64:
        self.data = np.complex128(
            self.data
        )  # everything is done assuming complex data
    # {{{ process arguments
    ...
    for j in axes:
        if j not in self.dimlabels:
            raise ValueError("the axis " + j + " doesn't exist")
    ...
    for j in range(0, len(axes)):
        ...
        if self.get_ft_prop(axes[j], "unitary") is None:  # has not been called
            ...
            if unitary[j] is None:
                ...
                raise ValueError(
                    "Call ft or ift with unitary only the first time, and it"
                    " will be set thereafter.\nOR if you really want to"
                    " override mid-way use"
    ...
    for j in range(0, len(axes)):
        ...
        if u is None:
            raise ValueError(
                "seems to be no axis for"
                + repr(axes[j])
                + "set an axis before you try to FT"
            )
        # }}}
        self.set_ft_prop(
            axes[j], ["start", "time"], u[0]
        ...
        du = check_ascending_axis(
            u, tolerance, "In order to perform FT or IFT"
        )
        self.set_ft_prop(axes[j], ["dt"], du)
        # }}}
        dv = (
            np.double(1) / du / np.double(padded_length)
        )  # so padded length gives the SW
        ...
        if desired_startpoint is not None:  # FT_start_freq is set
            if shift[j]:
                raise ValueError(
                    "you are not allowed to shift an array for"
            ...
            if automix:
                raise ValueError(
                    "You can't use automix and shift at the same time --> it"
                    " doesn't make sense"
                )
            n = padded_length
            p2_post = (
        ...
        if p2_post_discrepancy is not None:
            ...
            assert self.get_ft_prop(
                axes[j], ["time", "not", "aliased"]
            ), asrt_msg
            assert abs(p2_post_discrepancy) < abs(dv), (
                "I expect the discrepancy to be"
                " smaller than dv ({:0.2f}), but it's {:0.2f} -- what's going"
                " on??"
            ).format(dv, p2_post_discrepancy)
            phaseshift = self.fromaxis(
                axes[j],
            ...
            try:
                ...
                if self.data.dtype != "complex128":
                    raise TypeError(
                        "You tried to ft nddata that was of type "
                        + str(self.data.dtype)
                    )
                else:
                    raise TypeError(e)
        ...
        if cosine:
            self.data = np.fft.fft(self.data, axis=thisaxis) + np.ifft(
                self.data, axis=thisaxis
            )
            self.data *= 0.5
        else:
            self.data = np.fft.fft(self.data, axis=thisaxis)
        ...
        if automix:
            ...
            logging.debug(strm("I find carrier at", carrier))
