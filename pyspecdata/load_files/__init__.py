...
def search_filename(
    ...
    *   **Recommended** Point the source directories to a local directory on
    ...
    zenodo : str, optional
        ...
        used.  When using :func:`find_file`, this value is also passed to file
        format loaders that may need to fetch companion files, such as Bruker
        XEPR ``.DTA`` or ``.YGF`` files.  Use the command
        ``pyspecdata_zenodo <draft-id-or-title>`` to upload the
        ``data_files.csv`` generated by :func:`find_file` and
        :func:`search_filename`.
    zenodo_draft : bool, optional
        If ``True``, use Zenodo's authenticated draft-deposition API when
        downloading from ``zenodo``.  The default uses the public records API.
    """
...
def find_file(
    ...
    *   **Recommended** Point the source directories to a local directory on
    ...
    Note that all loaded files will be logged in the ``data_files.csv`` file
    in the directory that you run your python scripts from
    ...
    zenodo : str, optional
        ...
        locally, a file matching ``searchstring`` will be downloaded from this
        deposition instead of searching rclone remotes.  The value is also
        passed to file format loaders that may need companion files, such as
        Bruker XEPR ``.DTA`` or ``.YGF`` files.  Use
        ``pyspecdata_zenodo <draft-id-or-title>`` to upload the
        ``data_files.csv`` generated while finding data.
    zenodo_draft : bool, optional
        If ``True``, treat ``zenodo`` as an unpublished draft deposition and
        use the configured Zenodo token.  This is also passed to loaders that
        may need to fetch companion files from the same draft.
    """
