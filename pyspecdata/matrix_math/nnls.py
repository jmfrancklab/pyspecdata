...
def nnls(self, dimname, newaxis_dict, kernel_func, l=0):
    ...
    def demand_real(x, addtxt=""):
        if not x.dtype == np.float64:
            if x.dtype == np.complex128:
                ...
                raise ValueError(
                    "I expect double-precision floating point (np.float64), but you passed me data of dtype "
                    + str(x.dtype)
                    + "\n"
                    + addtxt
                )
    ...
    #        fit_dimaxes = [j.data]
    fit_axes = [
        self.__class__(fit_dimaxes[j], fit_dimnames[j])
    ...
    logger.debug(
        strm(
            "before mkd, np.shape of the data is",
            self.shape,
            "len of axis_coords_error",
            len(self.axis_coords_error),
        )
