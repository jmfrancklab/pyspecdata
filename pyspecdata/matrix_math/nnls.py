...
def nnls(self, dimname, newaxis_dict, kernel_func, l=0, default_cut=1e-2):
    ...
    if twoD:
        # {{{ prepping 2D
        K = K[0][:, newaxis, :, newaxis] * K[1][newaxis, :, newaxis, :]
        K = K.reshape(
            K[0].shape[0] * K[1].shape[0], K[0].shape[1] * K[1].shape[1]
        )
        logger.debug(
            strm(
                "Compressed K0, K[0], and K[1]:",
                [x.shape for x in (K, K[0], K[1])],
            )
        )
