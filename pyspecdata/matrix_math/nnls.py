...
def nnls(self, dimname, newaxis_dict, kernel_func, l=0):
    ...
    if isinstance(newaxis_dict, tuple):
        ...
        if isinstance(newaxis_dict[0], type(self)) and isinstance(
            newaxis_dict[1], type(self)
        ):
            assert (
    ...
    if tuple_syntax:
        ...
        logger.debug(
            strm("shape of fit dimension is", newaxis_dict[fitdim_name].shape)
        )
    ...
    if tuple_syntax:
        ...
        K1_ret = K1
        logger.debug(
            strm("K1 dimlabels", K1.dimlabels, "and raw shape", K1.data.shape)
        )
        K2 = kernel_func[1](data_axis2, fit_axis2).squeeze()
        K2_ret = K2
        logger.debug(
            strm("K2 dimlabels", K2.dimlabels, "and raw shape", K2.data.shape)
        )
        ...
        if l == "BRD":
            ...
            def mod_BRD(guess, maxiter=20):
                ...
                for iter in range(maxiter):
                    ...
                    if alpha_converged:
                        logger.debug(
                            strm("*** OPTIMIZED LAMBDA", lambda_update, "***")
                        )
                        break
        ...
        newshape.append(fit_axis1.shape[fitdim_name1])
        newshape.append(fit_axis2.shape[fitdim_name2])
        logger.debug(
            strm(
        ...
        newshape.append(fit_axis.shape[fitdim_name])
        logger.debug(
            strm(
                "before mkd, shape of the data is",
                self.shape,
        ...
        if not np.isscalar(residual):
            # make the residual nddata as well
            residual_nddata = self.shape.pop(fitdim_name).alloc(
