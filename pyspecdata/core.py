    ...
    if np.any(length_of_matching == 0):
        if drop_rows:
            if drop_rows == "return":
                ...
                print(
                    r"{\color{red}Warning! decorate\_rec dropped fields in the"
                    r" first argument",
                    lsafen(
                        repr(
                            list(
...
def plot(*args, **kwargs):
    ...
    ax, human_units, label_format_string, normalize, noerr, longest_is_x = (
        process_kwargs(
            [
                ("ax", plt.gca()),
                ("human_units", False),
                ...
                if chop:
                    ...
                    raise ValueError(
                        strm(
                            ...
                            "!=",
                            shapetocheckagainst,
                            "where all the shapes of the things you're trying"
                            " to concat are:",
                            shapes,
                        )
                    )
        ...
        if len(args) == 1:
            if isinstance(args[0], np.ndarray):
                if args[0].shape[2] != len(self.dimlabels):
                    raise ValueError(
                        "You must pass an N x M np.array, where M is the"
                        " number of dimensions in this np.array!"
                    )
            elif isinstance(args[0], dict):
                raise ValueError(
                    "should eventually support dictionaries (via fld), but"
                    " doesn't yet"
                )
            else:
                raise ValueError(
                    "I don't know what funny business you're up to passing"
                    " me a"
                    + repr(type(args[0]))
                )
        else:
            raise ValueError(
                "should eventually support np.array, label pair, but"
                " doesn't yet"
            )
