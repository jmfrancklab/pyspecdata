        ...
        try:
            ...
            raise RuntimeError(
                strm(
                    ...
                    " pieces of data" % (len(indices), len(mystructarray[thisfield])),
                    "indices:",
                    indices,
                    "data",
                    mystructarray[thisfield],
...
class nddata(object):
    ...
    def mkd(self, *arg, **kwargs):
        ...
        if len(arg) == 1:
            ...
            if len(input_list) != len(self.dimlabels):
                print(
                    r"{\color{red}WARNING! mkd error (John will fix this" r" later):}"
                )
                print(
    ...
    def rename(self, previous, new):
        ...
        all_ft_props = [j for j in self.other_info.keys() if j.startswith("FT")]
    ...
    def contiguous(self, lambdafunc, axis=None):
        ...
        **Currently only supported for 1D data**
        ...
        if mask[0]:  # because I want to indicate the boundaries of True, if I am
            # starting on True, I need to make 0 a boundary
            idx = np.r_[0, idx]
        if mask[-1]:  # If the end of mask is True, then I need to add a boundary there
            # as well
    ...
    def extend(self, axis, extent, fill_with=0, tolerance=1e-5):
        ...
        thismsg = (
            "In order to expand, the axis must be ascending (and equally" " spaced)"
        )
            ...
            if (
                ...
                if start_in_conj:
                    ...
                    raise ValueError(
                        ...
                        " have: " + self.ft_state_to_str(along_axis, propto_axis)
                    )
        if method == "fourier":
    ...
    def functional_form(self, sym_expr):
        ...
        if len(self.fit_axis) == 0:
            ...
            raise ValueError(
                ...
                " easyto change -- I see potential fit axes %s" % str(self.fit_axis)
            )
