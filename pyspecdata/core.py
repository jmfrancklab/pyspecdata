...
_figure_mode_setting = _my_config.get_setting(
...
)  # doesn't like to be imported from fornotebook as part of a *
from sympy.core import (
    Expr as sympy_expr,
)  # doesn't like to be imported from fornotebook as part of a *
...
def make_bar_graph_indices(
    ...
    if len(list_of_text_fields) > 0:
        ...
        )  # the return_index argument doesn't do what it's supposed to all the time, so I have to manually find the start indices, as given in the following line
        start_indices = [
        ...
        logger.debug(
            strm(
                "recursion depth is",
                recursion_depth,
                "and I just COMPLETED THE LOOP, which gives a list of index values like this",
                index_values,
            )
        )
        max_indices = max(
        ...
        ]  # if the bar is less than max indices, shift it over, so it's still in the center
        logger.debug(
            strm(
                "recursion depth is",
                recursion_depth,
        ...
        ]  # scale down, so the width from left edge of bar to right edge of largest bar runs 0--> 1
        logger.debug(
            strm(
                "recursion depth is",
                recursion_depth,
                "and I scaled down so each runs zero to one*(1-spacing) (centered) like this",
                index_values,
            )
        )
        ...
        logger.debug(
            strm(
                "hit innermost (no text labels left) and passing up a list of indices that looks like this:",
                r_[0:N],
            )
        )
        return r_[0:N], [""] * N
...
def textlabel_bargraph(
    ...
    for j, thisfield in enumerate(remaining_fields):
        ...
        try:
            ...
            raise RuntimeError(
                strm(
                    "Problem with bar graph: there are %d indices, but %d pieces of data"
...
def decorate_rec(xxx_todo_changeme2, xxx_todo_changeme3, drop_rows=False):
    ...
    if ((isinstance(b_ind, list)) and (isinstance(a_ind, list))) and (
        ...
        raise ValueError(
            "If you call a list for b_ind and/or a_ind, they must match in length!!!"
        )
    ...
    if np.any(length_of_matching == 0):
        if drop_rows:
            if drop_rows == "return":
                ...
                print(
                    r"{\color{red}Warning! decorate\_rec dropped fields in the first argument",
                    lsafen(
                        repr(
                            list(
                                zip(
...
def applyto_rec(myfunc, myarray, mylist):
    ...
    while len(myarray) > 0:
        ...
        # }}}
        logger.debug(
            strm(lsafen("(applyto rec): for row %d, I select these:" % j))
        )
        ...
        logger.debug(
            strm(lsafen("(applyto rec): other fields are:", other_fields))
        )
        ...
        logger.debug(
            strm(
                lsafen(
                    "(applyto rec): for row %d, I get this as a result:" % j,
                    newrow,
                )
            )
        )
        ...
        logger.debug(
            strm(lsafen("(applyto rec): the np.array is now", repr(myarray)))
        )
        j += 1
    # }}}
...
def meanstd_rec(myarray, mylist, standard_error=False):
    ...
    logger.debug(
        strm(lsafen("(meanstd rec): other fields are:", other_fields))
    )
    # {{{ make the list "combined", which I later concatenate
    j = 0
    while len(myarray) > 0:
        ...
        # }}}
        logger.debug(
            strm(lsafen("(meanstd rec): for row %d, I select these:" % j))
        )
        myarray_subset = myarray[mask]
        ...
        logger.debug(
            strm(
                lsafen(
                    "(meanstd rec): for row %d, I get this as a result:" % j,
                    newrow,
                )
            )
        )
        ...
        logger.debug(
            strm(lsafen("(meanstd rec): the np.array is now", repr(myarray)))
        )
        j += 1
    # }}}
    combined = np.concatenate(combined)
...
def explain_error(e):
    ...
    code_loc = strm(
        exc_tb.tb_frame.f_code.co_filename, "line", exc_tb.tb_lineno
    )
    return "\n> Original error (%s -- %s):\n" % (
        exc_type,
        code_loc,
    ) + "\n".join(
        [">\t" + j for j in str(e).split("\n")]
    )  # this indents
...
def h5searchstring(*args, **kwargs):
    ...
    **OR**
    ...
    if isinstance(value, str):
        raise ValueError(
            "string matching in pytables is broken -- search by hand and then use the index"
        )
    precision *= value
...
def h5child(thisnode, childname, clear=False, create=None):
    ...
    try:
        ...
        if create is False and not clear:
            raise RuntimeError(
                "Trying to grab a node that does not exist with create = False"
                + explain_error(e)
            )
        elif clear:
            childnode = None
...
def h5addrow(bottomnode, tablename, *args, **kwargs):
                ...
                if only_last:
                    logger.debug(
                        strm(
                            ...
                            lsafen(
                                len(matches),
                                "rows match your search criterion, returning the last row",
                            ),
                            "}",
                        )
                    )
    ...
    if len(args) == 1 and (isinstance(args[0], dict)):
        ...
        raise TypeError(
            "h5addrow takes either a dictionary for the third argument or a list for the third and fourth arguments"
        )
    try:
    ...
    if tableexists:
        try:
            ...
            print(
                lsafen(
                    "I'm about to flag an error, but it looks like there was an issue appending",
                    myrowdata,
                )
            )
            ...
            raise AttributeError(
                strm(
                    "Compare names and values table data vs. the row you are trying to add\n",
                    "\n".join(
                        map(
                            repr,
                            list(
...
def h5nodebypath(
    ...
    if h5path[0] in listdir(directory):
        ...
        if check_only:
            errmsg = log_fname(
                "missing_data_files", h5path[0], directory, err=True
            )
            raise AttributeError(
    ...
    try:
        ...
        raise IOError(
            "I think the HDF5 file has not been created yet, and there is a bug pytables that makes it freak out, but you can just run again."
            + explain_error(e)
        )
    # }}}
    ...
    for pathlevel in range(1, len(h5path)):  # {{{ step down the path
        ...
        try:
            ...
            raise IndexError(
                strm("Problem trying to load node ", h5path, explain_error(e))
            )
        # }}}
...
def h5attachattributes(node, listofattributes, myvalues):
    ...
    if isinstance(myvalues, nddata):
        ...
        raise TypeError(
            "I don't understand the type of myvalues, which much be a list or a nddata object, from which the attribute values are retrieved"
        )
...
def h5inlist(columnname, mylist):
    ...
    if np.any([type(x) in [np.double, np.float64] for x in mylist]):
        ...
        raise TypeError(
            "I can't figure out what to do with this list --> I know what to do with a list of numbers or a list of strings, but not a list of type"
...
def h5join(
    ...
    for thistableindex, thisstructarrayindex in zip(
        ...
        if isinstance(mystructarray[thisstructarrayindex][0], str):
            ...
            raise TypeError(
                "I don't know what to do with a structured np.array that has a row of type"
                + repr(type(mystructarray[thisstructarrayindex][0]))
            )
    # }}}
    search_string = [
        " & ".join(x) for x in zip(*tuple(search_string))
    ]  # this "and"s together the inner lists, since all conditions must be matched
    search_string = (
        "(" + "|".join(search_string) + ")"
    )  # then, it "or"s the outer lists, since I want to collect data for all rows of the table
    # }}}
    ...
    if pop_fields is not None:
        if select_fields is not None:
            raise ValueError(
                "It doesn't make sense to specify pop_fields and select_fields at the same time!!"
            )
...
def gridandtick(
    ...
    gridcolor=r_[0, 0, 0],
    formatonly=False,
    fixed_y_locator=None,
    use_grid=True,
    spines=None,
    y=True,
):
    defaultMajorLocator = lambda: mticker.MaxNLocator(
        min_n_ticks=4, steps=[1, 2, 5, 10]
    )
    ...
    if not formatonly:
        ...
        widthexp = np.floor(np.log(width) / np.log(10.0)) - 1
        ...
        if y:
            logarithmic = (
                True if ax.get_yaxis().get_scale() == "log" else False
            )
            # {{{ y ticks
    ...
    if use_grid:
        ax.yaxis.grid(
            use_grid, which="major", color=gridcolor, alpha=0.15, linestyle="-"
        )
        ax.xaxis.grid(
            use_grid, which="major", color=gridcolor, alpha=0.15, linestyle="-"
        )
...
def othergridandtick(
    ...
    if x:
        ...
        )  # could use multiplelocator if it keeps try to do multiples of 2
...
def autolegend(*args, **kwargs):
    ...
    for ax in ax_list:
        ...
        if lg is None:
            raise ValueError(
                "Warning! you called autolegend, but you don't seem to have anything labeled!!"
            )
        else:
            lg.get_frame().set_alpha(alpha)
    if match_colors:
        for line, txt in zip(
            lg.get_lines(), lg.get_texts()
        ):  # from http://stackoverflow.com/questions/13828246/matplotlib-text-color-code-in-the-legend-instead-of-a-line
...
def autopad_figure(pad=0.2, centered=False, figname="unknown"):
    ...
    def on_draw(event):
                ...
                for label in labels:
                    if isinstance(label, Line2D):
                        ...
                        try:
                            ...
                            warnings.warn(
                                "I wasn't able to run autopad on figure"
                                + figname
                                + "\nGetting window extent throws error"
                                + str(e)
                            )
        ...
        if len(spkwargs) > 0:
            if (
                centered
                and "left" in list(spkwargs.keys())
                and "right" in list(spkwargs.keys())
            ):
            ...
            try:
                ...
                raise RuntimeError(
                    "failed to adjust subplots spwargs = ", spkwargs
                )
...
def plot_color_counter(*args, **kwargs):
    ...
    if len(args) > 0:
        if LooseVersion(matplotlib.__version__) >= LooseVersion("1.5"):
            ...
            try:
                ax._get_lines.count = args[
                    0
                ]  # set the value of the color counter
            except:
                ax._get_lines.color_cycle = args[
                    0
                ]  # set the value of the color counter
            retval = args[0]
    else:
...
def contour_plot(
    xvals, yvals, zvals, color="k", alpha=1.0, npts=300, **kwargs
):
    ...
    zi = scipy_griddata((xvals, yvals), zvals, (xi[None, :], yi[:, None]))
    zi_min = zi[np.isfinite(zi)].min()
    zi_max = zi[np.isfinite(zi)].max()
    levels = r_[zi_min:zi_max:40j]
    CS = contour(xi, yi, zi, levels, colors=color, alpha=0.25 * alpha)
    oldspacing = levels[1] - levels[0]
    levels = r_[zi_min : zi_max : oldspacing * 5]
    try:
        CS = contour(xi, yi, zi, levels, colors=color, alpha=alpha, **kwargs)
    except Exception as e:
        raise Exception(
            strm(
                "Is there something wrong with your levels?:",
                levels,
                "min z",
                zi_min,
                "max z",
...
def plot_updown(data, axis, color1, color2, symbol="", **kwargs):
    ...
    if len(kwargs) > 0 and "label" in list(kwargs.keys()):
        kwargs.pop("label")  # if I'm doing a legend, I want it on the first
            ...
            if len(testdata.dimlabels) > 1:
                ...
                if self.current in list(self.units.keys()):
                    ...
                    if (
                        ...
                        raise ValueError(
                            "the units don't match (old units %s and new units %s)! Figure out a way to deal with this!"
                            % (theseunits, self.units[self.current])
                        )
                else:
                    if isinstance(testdata, nddata):
                ...
                if self.current in list(self.units.keys()):
                    ...
                    if theseunits != testunits:
                        if (
                            ...
                            raise ValueError(
                                "the units don't match (old units %s and new units %s)! Figure out a way to deal with this!"
                                % (self.units[self.current], theseunits)
                            )
                else:
...
def text_on_plot(x, y, thistext, coord="axes", **kwargs):
    ...
    if "match_data" in list(kwargs.keys()):
        if isinstance(kwargs["match_data"], list):
            color = kwargs["match_data"][
                -1
            ].get_color()  # get the color of the last line
        elif kwargs["match_data"].get_plot_color() is not None:
            color = kwargs[
                "match_data"
            ].get_plot_color()  # don't know when this works, but apparently, it does!
        if color is not None:
            newkwargs.update({"color": color})
        else:
            raise ValueError(
                "You passed match_data to text_on_plot, but I can't find a color in the object"
            )
...
def plot(*args, **kwargs):
    ...
    if (
        (myx is not None) and (len(myx) > 1) and all(myx > 0)
    ):  # by doing this and making myplotfunc global, we preserve the plot style if we want to tack on one point
        try:
    ...
    if len(np.shape(myy.squeeze())) > 1 and np.sum(
        ...
        for j in range(0, myy.shape[1]):
            ...
            if np.any(np.isinf(myy)):
                myy[
                    np.isinf(myy)
                ] = NaN  # added this to prevent an overflow error
            try:
...
class nddata(object):
        ...
        if len(args) > 1:
            ...
            if len(args) == 2:
                if len(args[0].shape) == 1 and isinstance(args[1], str):
                    ...
                    self.labels(
                        args[1], args[0].copy()
                    )  # needs to be a copy, or when we write data, we will change the axis
                    ...
                    raise ValueError(
                        "You can pass two arguments only if you pass a 1d np.ndarray and a name for the axis"
                    )
            elif len(args) == 3:
    ...
    def __my_init__(
        ...
        if ft_start_time is not None:
            raise ValueError(
                "ft_start_time is obsolete -- you will want to pass a float value to the shift keyword argument of either .ft() or .ift()"
            )
        ...
        if not (isinstance(dimlabels, list)):
            raise TypeError(
                strm(
                    "you provided a multi-dimensional np.ndarray but a set of dimension labels of type",
                    type(dimlabels),
                    "if you want a 1D nddata, give a 1D np.array, or if you want a ND nddata, give a list of dimensions",
                )
            )
        try:
    ...
    def sort_and_xy(self):
        ...
        if len(self.dimlabels) > 2:
            raise ValueError(
                "I don't know how to handle something with more than two dimensions for a surface plot!"
            )
        # {{{ shared to both
    ...
    def matrices_3d(
        ...
        if np.any(this_size > max_dimsize):
            print(
                lsafen(
                    "Warning! The data is big (%s), so I'm automatically downsampling"
                    % (ndshape(self))
                )
            )
            for j in np.where(this_size > max_dimsize):
                downsampling = int(
                    np.ceil(np.double(this_size[0]) / max_dimsize)
                )
                downsampling = int(
                    np.ceil(np.double(this_size[1]) / max_dimsize)
                )
            ...
            print(
                lsafen(
                    "I reduced to a max of max_dimsize = %d so the data is now %s"
        ...
        **kwargs,
        ...
        if light == True:
            light = [
                0,
                0,
            ]  # I think this is 45 degrees up shining down from the left of the y axis
        if not onlycolor:
            if ax is None:
    ...
    def normalize(
        ...
        first_point = self[
            axis, 0:1
        ].copy()  # this makes another instance that contains just the first point, for error propagation
        B = sparse.lil_matrix((n, n))
        B.setdiag(1.0 / x)
        B[0, :] = -x / (
            x[0] ** 2
        )  # Sparse seems to only support row assignment, so make the transpose to give it what it wants
        B[0, 0] = 0.0
        B = B.T
    ...
    def mkd(self, *arg, **kwargs):
        ...
        if len(arg) == 1:
            ...
            if len(input_list) != len(self.dimlabels):
                print(
                    r"{\color{red}WARNING! mkd error (John will fix this later):}"
                )
                print(
                    "When making a dictionary with mkd, you must pass a list that has one element for each dimension!  dimlabels is "
                    + repr(self.dimlabels)
                    + " and you passed "
                    + repr(arg)
                    + "\n\n"
            ...
            if not give_None:
                raise ValueError(
                    "You can't tell me not to give none and then not pass me anything!!"
                )
            return dict(zip(self.dimlabels, [None] * len(self.dimlabels)))
        else:
            raise ValueError(
    ...
    def get_error(self, *args):
        ...
        if len(args) == 0:
            ...
            if isinstance(thearg, np.str_):
                thearg = str(
                    thearg
                )  # like in the other spot, this became necessary with some upgrade, though I'm not sure that I should maybe just change the error functions to treat the numpy string in the same way
            if isinstance(thearg, str):
                if len(self.axis_coords_error) == 0:
                    self.axis_coords_error = [None] * len(
                        self.dimlabels
                    )  # is we have an np.empty axis_coords_error, need to fill with None's
                try:
    ...
    def __rpow__(self, arg):
        ...
        logger.debug(
            "error propagation for right power not currently supported (do you need this, really?)"
        )
        assert np.isscalar(arg) or isinstance(
            arg, np.ndarray
        ), "currently right power only supported for np.ndarray and scalars -- do you really need something else??"
    ...
    def __pow__(self, arg):
        if arg == -1:
            ...
            if self.get_error() is not None:
                raise ValueError(
                    strm(
                        "nothing but -1 and 2 supported yet! (you tried to raise to a power of "
                        + repr(arg)
                        + ")"
                    )
                )
    ...
    def __div__(self, arg):
        ...
        try:
            Rerr = sqrt(
                np.real(Rerr)
            )  # convert back to stdev --> note that this has problems with complex numbers, hence the "abs" above
        except AttributeError as e:
            raise AttributeError(
    ...
    def aligndata(self, arg):
        ...
        if np.isscalar(arg) or isinstance(arg, np.ndarray):
            ...
            for j in index_dims:  # find dimension of matching length
                ...
                if len(match_dims) != len(index_dims):
                    raise ValueError(
                        "you seem to by multiplying by something with an 'INDEX' data and something that doesn't have that -- is this really what you want?  (this is commonly produced by multiplying a mismatched np.ndarray by an nddata)"
                    )
        ...
        new_arg_labels = [
            x for x in newdims if x in arg.dimlabels
        ]  #  only the labels valid for arg, ordered
        #                         as they are in newdims
        argshape = list(
            np.ones(len(newdims), dtype=np.int64)
    ...
    def polyfit(self, axis, order=1, force_y_intercept=None):
        ...
        if force_y_intercept is not None:
            startingpower = 1
        L = np.concatenate(
            [x**j for j in range(startingpower, order + 1)], axis=1
    ...
    def argmax(self, *args, **kwargs):
        ...
        for j in range(0, len(axes)):
            try:
                ...
                raise
            if raw_index:
                self.data = np.argmax(self.data, axis=thisindex)
            else:
                if self.axis_coords[thisindex] is None:
                    raise ValueError(
                        "It doesn't make sense to call argmax if you have removed the axis coordinates! (getaxis yields None for %s"
                        % thisindex
                    )
                self.data = self.axis_coords[thisindex][
                    np.argmax(self.data, axis=thisindex)
                ]
            self._pop_axis_info(thisindex)
        return self
    ...
    def argmin(self, *axes, **kwargs):
        ...
        for j in range(0, len(axes)):
            try:
                ...
                raise
            if raw_index:
                self.data = np.argmin(self.data, axis=thisindex)
            else:
                self.data = self.axis_coords[thisindex][
                    np.argmin(self.data, axis=thisindex)
                ]
            self._pop_axis_info(thisindex)
        return self
    ...
    def cdf(self, normalized=True, max_bins=500):
        ...
        bins, vals = histogram(self.data, bins=n_bins)
    ...
    def mean(self, *args, **kwargs):
        ...
        if "return_error" in kwargs:
            raise ValueError(
                "return_error kwarg no longer used -- use std kwarg if you want to set the error to the std"
            )
        ...
        for j in range(0, len(axes)):
            ...
            if return_error:  # since I think this is causing an error
                thiserror = std(self.data, axis=thisindex)
    ...
    def cropped_log(self, subplot_axes=None, magnitude=4):
        ...
        if subplot_axes is None:  # then do all
            self.data -= (
                self.data.flatten().max() - magnitude
        ...
        if len(args) > 1:
            ...
            try:
                ...
                if not isinstance(axis, str):
                    raise ValueError(
                        'The format of run is run(func,"axisname"), but you didn\'t give a string as the second argument -- maybe you fed the arguments backwards?'
                    )
    ...
    def run_nopop(self, func, axis):
        ...
        if numnonoptargs == 2:
            ...
            if "axes" in kwargnames:
                ...
                raise ValueError(
                    "Your function doesn't have axis or axes as a keyword argument!"
                )
        else:
            if numnonoptargs == 1:
                ...
                if "axis" in kwargnames:
                    ...
                    raise ValueError(
                        "Your function doesn't have axis or axes as a keyword argument! The number of non-optional arguments are %s. The keyword arguments are %s"
                ...
                if len(followword) > 0:
                    if not (k > -1) and (
                        ...
                        if len(followword) > 1:
                            ...
                            axis_name = (
                                axis_name[: j + 1] + "{" + axis_name[j + 1 :]
                            )
                        else:
                            axis_name = (
                                axis_name[0 : j + 2] + "$" + axis_name[j + 2 :]
                            )
                        axis_name = "$" + axis_name
            if isft:
    ...
    def nnls(self, dimname, newaxis_dict, kernel_func, l=0):
        ...
        if isinstance(dimname, tuple):
            ...
            assert type(dimname[0]) and isinstance(
                dimname[1], str
            ), "first argument is tuple of two dimension names"
        else:
            assert isinstance(
                dimname, str
            ), "first argument is dimension name or tuple of two dimension names"
        if isinstance(newaxis_dict, tuple):
            assert len(newaxis_dict) == 2, "tuple of two nddatas only"
            if isinstance(newaxis_dict[0], nddata) and isinstance(
                newaxis_dict[1], nddata
            ):
                assert (
                    len(newaxis_dict[0].dimlabels)
                    and len(newaxis_dict[1].dimlabels) == 1
                ), "currently only set up for 1D"
            ...
            assert (
                len(newaxis_dict.dimlabels) == 1
            ), "currently only set up for 1D"
        else:
            raise ValueError(
                "second argument is dictionary or nddata with new axis, or tuple of nddatas with new axes"
            )
        ...
        if tuple_syntax:
            ...
            logger.debug(
                strm(
                    "shape of fit dimension is",
                    newaxis_dict[fitdim_name].shape,
                )
            )
        ...
        if tuple_syntax:
            ...
            logger.debug(
                strm("Uncompressed SVD K1:", [x.shape for x in (U1, S1, V1)])
            )
            logger.debug(
                strm("Uncompressed SVD K2:", [x.shape for x in (U2, S2, V2)])
            )
            default_cut = 1e-2
            ...
            logger.debug(
                strm(
                    "Compressed K0, K1, and K2:",
                    [x.shape for x in (K, K1, K2)],
                )
            )
            ...
            if l == "BRD":
                ...
                def d_chi(x_vec, val):
                    return (
                        np.dot(dd_chi(G(x_vec), val**2), x_vec)
                        - data_fornnls[:, newaxis]
                    )
            ...
            logger.debug(
                strm(
                    "coming back from fortran, residual type is",
                    type(residual),
                )
                + strm(
                    residual.dtype if isinstance(residual, np.ndarray) else ""
                )
            )
            newshape = []
            ...
            logger.debug(
                strm("K dimlabels", K.dimlabels, "and raw shape", K.data.shape)
            )
            self.reorder(
                dimname, first=False
            )  # make the dimension we will be regularizing innermost
            logger.debug(
                strm(
            ...
            if not np.isscalar(residual):
                # make the residual nddata as well
                residual_nddata = (
                    ndshape(self).pop(fitdim_name).alloc(dtype=residual.dtype)
                )
                residual_nddata.data[:] = residual[:]
            else:
                residual_nddata = residual
    ...
    def histogram(*args, **kwargs):
        ...
        if len(kwargs) > 0:
            raise ValueError(
                "I don't understand the arguments:" + repr(kwargs)
            )
        if len(args) == 1:
            if isinstance(args[0], np.ndarray):
                if args[0].shape[2] != len(self.dimlabels):
                    raise ValueError(
                        "You must pass an N x M np.array, where M is the number of dimensions in this np.array!"
                    )
            elif isinstance(args[0], dict):
                raise ValueError(
                    "should eventually support dictionaries (via fld), but doesn't yet"
                )
            else:
                raise ValueError(
                    "I don't know what funny business you're up to passing me a"
                    + repr(type(args[0]))
                )
        else:
            raise ValueError(
                "should eventually support np.array, label pair, but doesn't yet"
            )
    ...
    def interp(
            ...
            if past_bounds is None:
                ...
                if isinstance(past_bounds, tuple):
                    if len(past_bounds) == 2:
                        ...
                        raise TypeError(
                            "If you pass axisvalues as a tuple, it must be of length 2!"
                        )
                else:
        ...
        if thiserror is not None:
            rerrvar = local_interp_func(
                rerrvar, kind="linear"
            )  # calculate the error variance of the real part, use linear to avoid nan problems
            if thiserror[0].dtype == "complex128":
        ...
        **Currently only supported for 1D data**
        ...
        if axis is None:
            if len(self.dimlabels) == 1:
                ...
                raise TypeError(
                    "If there is more than one dimension, `axis` must be set to something other than ``None``"
                )
        else:
        ...
        if axis is None:
            (idx,) = np.diff(
                mask
            ).nonzero()  # this gives a list of indices for the boundaries between true/false
        else:
            (idx,) = np.diff(
        ...
        if mask[
            0
        ]:  # because I want to indicate the boundaries of True, if I am starting on True, I need to make 0 a boundary
            idx = np.r_[0, idx]
        if mask[
            -1
        ]:  # If the end of mask is True, then I need to add a boundary there as well
    ...
    def to_ppm(self, axis="t2", freq_param="SFO1", offset_param="OFFSET"):
        ...
        self.setaxis(axis, lambda x: x / sfo1).set_units(axis, "ppm")
    ...
    def add_noise(self, intensity):
        ...
        self.data += (
            box_muller(self.data.size, return_complex=return_complex).reshape(
                self.data.shape
            )
            * intensity
        )
    ...
    def labels(self, *args):
        ...
        if not isinstance(listofstrings, list):
            raise TypeError(
                "the arguments passed to the .labels() method must be a list of the axis names followed by the list of the axis arrays"
            )
            ...
            if listofaxes[j] is None:
                ...
                if type(listofaxes[j]) not in [np.ndarray, list]:
                    raise TypeError(
                        ...
                        + " for the axis "
                        + listofstrings[j]
                        + " to the labels method, which you can't do --> it must be an nddata"
                    )
                if (
                    len(listofaxes[j]) != ndshape(self)[listofstrings[j]]
                ) and (len(listofaxes[j]) != 0):
                    raise IndexError(
                        "You're trying to attach an axis of len %d to the '%s' dimension, which has %d data points (shape of self is %s)"
                        % (
                            len(listofaxes[j]),
                            listofstrings[j],
    ...
    def check_axis_coords_errors(self):
        if len(self.axis_coords_error) > len(self.dimlabels):
            raise ValueError(
                "this failed because there are more sets of axis errors than there are axes!\nlen(axis_coords_error) = %s\naxes = %s"
    ...
    def fromaxis(self, *args, **kwargs):
        ...
        * ``self.fromaxis(inputsymbolic)``: Evaluate `inputsymbolic` and load the result into `retval.data`
            ...
            if isinstance(args[0], str):
                ...
                if overwrite:
                    ...
                    if self.axis_coords_units is None:
                        ...
                        retval.axis_coords_units = [
                            self.axis_coords_units[self.axn(axisname)]
                        ]
        ...
        if issympy(func):
            logging.debug(
                strm(
                    "about to run sympy lambdify, symbols_in_func is",
                    symbols_in_func,
                )
            )
            try:
                lambdified_func = lambdify(
            ...
            raise ValueError(
                "I can't interpret the second argument as a function! It is type "
                + str(type(func))
            )
        ...
        if issympy(retval):
            raise RuntimeError(
                "The sympy function that you passed doesn't match the automatically generated axis variables (obtained by mapping sympy.var onto the axis variables, without any kwargs). The atoms left over are:\n"
                + str(func.atoms)
            )
    ...
    def extend(self, axis, extent, fill_with=0, tolerance=1e-5):
        ...
        if extent < u[0]:
            ...
            raise RuntimeError(
                "extent ({:g}) needs to be further than the bounds on '{:s}', which are {:g} and {:g}".format(
                    extent, axis, u[0], u[-1]
                )
            )
    ...
    def setaxis(self, *args):
        ...
        * ``self.fromaxis(inputsymbolic)``: Evaluate `inputsymbolic` and load the result into the axes, appropriately
        ...
        if len(args) == 2:
            ...
            assert (
                len(axis) == 1
            ), "currently only supported for 1 axis at a time -- if you want to do for more than one axis, please create a pull request with an example"
            axis = axis[0]
        else:
            raise ValueError(
        ...
        if axis == "INDEX":
            raise ValueError(
                "Axes that are called INDEX are special, and you are not allowed to label them!"
            )
        ...
        if value is None:
            ...
            assert a == b, (
                "Along the axis %s, the length of the axis you passed (%d) doesn't match the size of the data (%d)."
                % (axis, a, b)
            )
    ...
    def shear(
        ...
        if not (
            ...
            if (
                ...
                if start_in_conj:
                    raise ValueError(
                        "if you pass start_in_conj, the two dimensions need to be in conjugate domains, but you have: "
                        + self.ft_state_to_str(along_axis, propto_axis)
                    )
                else:
                    raise ValueError(
                        "(unless you intended to pass start_in_conj) the two dimensions need to be in the same domain, but you have: "
                        + self.ft_state_to_str(along_axis, propto_axis)
                    )
        if method == "fourier":
    ...
    def smoosh(self, dimstocollapse, dimname=0, noaxis=False):
        ...
        assert (type(dimstocollapse) in [list, tuple]) and len(
            ...
            "What?? You must try to collapse more than one dimension!! -- you claim you want to collapse '%s'"
            % str(dimstocollapse)
        )
        ...
        ]  # specifically, I am only concerned with the np.ones I am collapsing that have labels
        if noaxis:
            ...
            if len(axes_with_labels_dtype) > 0:
                ...
                raise ValueError(
                    "You requested that smoosh generate an axis, but I don't know what dtype to assign to it (what fields to use).  This is likely because you don't have axes assigned to the dimensions you're trying to smoosh.  Consider calling smoosh with noaxis=True, instead"
                )
    ...
    def chunk(self, axisin, *otherargs):
        ...
        if len(otherargs) == 2:
            ...
            if isinstance(otherargs[0], list):
                ...
                if (
                    ...
                    raise ValueError(
                        """In order for chunk to be called with
                            ...
                            %d) is the number of dimensions you are trying to chunk into"""
        ...
        if np.prod(shapesout) != ndshape(self)[axisin]:
            raise ValueError(
                "The size of the axis (%s) you're trying to split (%s) doesn't match the size of the axes you're trying to split it into (%s = %s)"
                % (
                    repr(axisin),
    ...
    def chunk_auto(self, axis_name, which_field, dimname=None):
        ...
        )  # we are essentially creating a hash table for the axis.  According to numpy documentation, the hash indices that this returns should also be sorted sorted.
        ...
        if all(index_count == index_count[0]):
            logger.debug(
                strm(
                    "(chunk auto) Yes, there are equal numbers of all unique new_axis! (Each element of the hash table has been indexed the same number of times.)"
                )
            )
            # }}}
            ...
            if not (self.get_error(axis_name) is None):
                raise ValueError(
                    "Until I do the structured np.array upgrade chunk_auto will not be able to deal with an axis that has errors."
                )
            # }}}
            ...
            ]  # this is the memory address inside the new data (where stuff goes)
            copy_from_slice = len(current_shape) * [
                slice(None, None, None)
            ]  # this is the memory address inside the old data (where stuff comes from)
            # }}}
            if has_data_error:
            ...
            if (
            ...
            ):  # then this is a "valid" axis -- because, for each position of the new axis, there is only one value of the remainder axis
                ...
                if (
                ...
                ):  # only one field, which by the previous line will be named appropriately, so drop the structured np.array name
                    ...
                    )  # probably more efficiently done with a view, but leave alone for now
            ...
            raise ValueError(
                ...
                + "', the field '"
                + which_field
                + "' does not represent an axis that is repeated one or more times!  The counts for how many times each element along the field is used is "
                + repr(index_count)
            )
            return
    ...
    def __setitem__(self, key, val):
        ...
        if isinstance(key, np.ndarray):  # if selector is an np.ndarray
            ...
            if key.dtype is not np.dtype("bool"):
                raise ValueError(
                    "I don't know what to do with an np.ndarray subscript that has dtype "
                    + repr(key.dtype)
                )
    ...
    def __getitem__(self, args):
        if isinstance(args, type(emptyfunction)):
            ...
            if len(newdata.dimlabels) == 1:
                ...
                raise ValueError(
                    "I don't know how to do this for multidimensional data yet!"
                )
            return newdata
            # }}}
            ...
            if isinstance(A, nddata) and A.data.dtype is np.dtype("bool"):
                ...
                if len(nonsingleton) != 1:
                    raise ValueError(
                        "To index with an nddata, you must have only one dimension"
                    )
                else:
                    self.setaxis(
                        nonsingleton[0],
            ...
            ]  # generate the new list of labels, in order, for all dimensions that are not indexed by a scalar
    ...
    def _possibly_one_axis(self, *args):
        ...
        if len(args) == 0:
            if len(self.dimlabels) == 1:
                ...
                raise ValueError(
                    "If you have more than one dimension, you need to tell me which one!!"
                )
        return axes
    ...
    def _parse_slices(self, args):
        ...
        while j < len(args):
            if isinstance(args[j], str):  # works for str and np.str_
                ...
                if isinstance(dimname, np.str_):
                    dimname = str(
                        dimname
                    )  # on upgrading + using on windows, this became necessary, for some reason I don't understand
        ...
        logger.debug(
            strm(
                "Here is the sensible list:",
                [pprint(j) for j in sensible_list],
            )
        )
        # }}}
        ...
        if isinstance(args[0], str) or isinstance(args[0], slice):
            ...
            if len(self.axis_coords) > 0:
                ...
                logger.debug(
                    strm(
                        "length of axis_coords not greater than 0, generated dictionary",
                        axesdict,
                    )
                )
            # }}}
            ...
            for thistuple in sensible_list:
                ...
                if thisop == hash("np"):
                    ...
                    if not all(temp * np.sign(temp[0]) > 0):
                        raise ValueError(
                            strm(
                                "you can only use the range format on data where the axis is in consecutively increasing or decreasing order, and the differences that I see are",
                                temp * np.sign(temp[0]),
                            ),
                            "if you like, you can still do this by first calling .sort( on the %s axis"
                            % thisdim,
                        )
                    if np.sign(temp[0]) == -1:
                    ...
                    if len(thisargs) > 2:
                        raise ValueError(
                            "range with more than two values not currently supported"
                        )
                    elif len(thisargs) == 1:
                        temp_low = thisargs[0]
                        temp_high = inf
                    ...
                    if temp_low == inf:
                        ...
                        if temp_low >= len(thisaxis):
                            raise ValueError(
                                "the lower value of your slice on the %s axis is higher than the highest value of the axis coordinates!"
                                % thisdim
                            )
                        logger.debug(
                            strm(
                    ...
                    if temp_high == inf:
                        temp_high = len(
                            thisaxis
                        )  # not an exact match (inf doesn't match the index), so needs to be inclusive already
                    elif temp_high == -inf:
                        raise ValueError(
                            strm(
                                "this is not going to work -- I interpret range",
    ...
    def hdf5_write(self, h5path, directory="."):
            ...
            if self.getaxis(thisax) is None or len(self.getaxis(thisax)) == 0:
                raise ValueError(
                    strm(
                        "The axis",
                        thisax,
                        "appears not to have a label!  I refuse to save data to HDF5 if you do not label all your axes!!",
                    )
                )
        ...
        try:
            ...
            if "data" in mydataattrs:
                ...
                raise ValueError(
                    "I can't find the data object when trying to save the HDF5 file!!"
                )
            # }}}
            ...
            if "axis_coords" in myaxisattrs:
                if len(self.axis_coords) > 0:
                    ...
                    for j, axisname in enumerate(
                        ...
                        )  # collect the attributes for this dimension and their values
                        logger.debug(
                            strm(
                                lsafe(
                                    "for axis",
                        ...
                        if (
                            ...
                            if (
                                ...
                                > 0
                            ):  # this is needed to avoid all errors, though I guess I could use try/except
                                thistable = rec.fromarrays(
                                    [
            ...
            if len(myotherattrs) > 0:
                ...
                test = repr(
                    bottomnode
                )  # somehow, this prevents it from claiming that the bottomnode is None --> some type of bug?
                h5attachattributes(
                    bottomnode,
                    [
                        j
        ...
        if len(mydimlabels) > 1:
            ...
            try:
                self.data = self.data.reshape(
                    tuple([len(self.getaxis(x)) for x in mydimlabels])
                )
            except:
                raise RuntimeError(
                    strm(
        ...
        try:
            if format == 0:
                try:
                    ...
                    raise TypeError(
                        ...
                        + repr(dtype)
                        + ", which was likely not understood (you also passed a shape of "
            ...
            raise TypeError(
                strm(
                    ...
                    "this probably means that you swapped the size and name arguments -- ",
                    self.shape,
...
class subplot_dim:
    ...
    def set(self, args, x="", g=True, y="", t="", a=""):
        if isinstance(args, int):
            ...
            number = args[0]
            mydims = (
                self.num * r_[1, break_into, 1]
                + r_[0, 0, break_into * (number - 1) + whichsmall]
            )
            try:
...
def fa(input, dtype="complex128"):  # make a fortran array
    ...
    )  # will need transpose reverses the dimensions, since the bracketing still works in C order (inner is last index), but F tells it to store it appropriately in memory
...
def pinvr(C, alpha):
    ...
    if np.any(~np.isfinite(S)):
        raise ValueError(
            "pinvr error, problem with S/(S^2+alpha^2) --> set your regularization higher"
        )
    return np.dot(np.conj(transpose(V)), np.dot(S, np.conj(transpose(U))))
...
def spectrogram(waveform, f_start, f_stop, npoints_fdom=40, tdom_div=2):
    npoints_tdom = (
        waveform.len / tdom_div
    )  # this seems to be more legible than above
    ...
    waveform.def_filter(
        sigma, npoints_tdom
    )  # define the filter and number of points for the spectrogram windowing (define the filter such that the points are spaced sigma apart)
    ...
    )  # now do an imagehsv (see if we can make imagerybw) plot of the resulting spectrogram
    return plt.gca()
...
class fitdata(nddata):
    ...
    def function_string(self):
        ...
        return (
            r"$f(%s)=" % (sympy_latex(sympy_symbol(self.fit_axis)))
            + retval
            + r"$"
        )
    ...
    def functional_form(self, sym_expr):
        ...
        if len(self.fit_axis) == 0:
            raise ValueError(
                "I can't find any variables that might correspond to a dimension you want to fit along."
                "The variables are",
                symbols_in_expr,
            ...
            raise ValueError(
                "currently only 1D fitting is supported, though this should be easy"
        ...
        self.symbolic_vars = [
            sympy_var(j, real=True) for j in self.symbolic_vars
        ]
        ...
        def fn(p, x):
            ...
            assert len(p) == len(
                self.symbolic_vars
            ), "length of parameter passed to fitfunc doesn't match number of symbolic parameters"
    ...
    def analytical_covariance(self):
        ...
        if dirproductform:
            ...
            try:
                covarmat = np.dot(
                    pinv(fprime_prod), (sigma**2).reshape(-1, 1)
                )
            except ValueError as e:
                raise ValueError(
                    strm(
    ...
    def copy(
        self,
    ):  # for some reason, if I don't override this with the same thing, it doesn't override
        namelist = []
        vallist = []
        for j in dir(self):
    ...
    def linear(self, *args, **kwargs):
        ...
        if len(args) == 1:
            ...
            )  # if we pass an argument, return the function across the entire time axis passed
        else:
            return self.linfunc(
    ...
    def fit(self, set_what=None, set_to=None, force_analytical=False):
        ...
        p_out, np.cov, infodict, mesg, success = leastsq(
        ...
        # {{{ just give various explicit errors
            ...
            if not isinstance(x, np.ndarray) and not isinstance(y, np.ndarray):
                raise TypeError(
                    strm(
                        "leastsq failed because the two arrays",
                        "aren't of the right",
                        "type",
                        "type(x):",
                        type(x),
                ...
                if np.any(np.shape(x) != np.shape(y)):
                    raise RuntimeError(
                        strm(
                            "leastsq failed because the two arrays do"
            ...
            raise TypeError(
                strm(
                    "leastsq failed because of a type error!",
                    "type(x):",
                    showtype(x),
                    "type(y):",
                    showtype(y),
            ...
            if mesg.find("maxfev"):
                ...
                if success != 1:
                    if mesg.find("two consecutive iterates"):
                        print(
                            r"{\Large\color{red}{\bf Warning data is not fit!!! output shown for debug purposes only!}}",
                            "\n\n",
                        )
                        print(
                        ...
                        if "ipvt" in infodict_keys:
                            ...
                            ] = "ipvt, an integer np.array of length N which defines a permutation matrix, p, such that fjac*p = q*r, where r is upper triangular with diagonal elements of nonincreasing magnitude.  Column j of p is column ipvt(j) of the identity matrix"
                        ...
                        raise RuntimeError(
                            strm(
                                "leastsq finished with an error message:", mesg
                            )
                        )
                    # }}}
            else:
                raise RuntimeError(
                    strm("leastsq finished with an error message:", mesg)
                )
        else:
            logger.debug(
                "Fit finished successfully with a code of %d and a message ``%s''"
                % (success, mesg)
            )
        ...
        if len(symbols_not_present) > 0:
            raise ValueError(
                strm(
                    ...
                    symbols_not_present,
                    "but I can't find this in the symbols for the fitting function, which are",
                    self.symbolic_vars,
                )
            )
    ...
    def guess(self, use_pseudoinverse=False):
        ...
        if use_pseudoinverse:
            ...
            for j in range(0, numguesssteps):
                ...
                logger.debug(
                    strm(
                        "\n\n.core.guess) value of residual before regularization %d:"
                        % j,
                        thisresidual,
                    )
                )
                ...
                logger.debug(
                    strm(
                        "\n\n.core.guess) new value of guess after regularization:",
                        lsafen(newguess),
                    )
                )
                logger.debug(
                    strm(
                        "\n\n.core.guess) value of residual after regularization:",
                        thisresidual,
                    )
                )
            return thisguess
