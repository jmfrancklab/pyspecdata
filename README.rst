...
.. If you are installing from github (or generally using setuptools -- *i.e.* ``python setup.py install`` or ``python setup.py develop``)
...
(typically ``pyspecdata`` underneath where you called the clone
command)
and do
``python setup_paramset.py develop``
(*note that this is an unusual command, but necessary*)
followed by
``python setup.py develop``

Once you have done this, you can run
``pyspecdata_dataconfig``
to set up an example config file
...
1. Install a good Python 2.7 distribution if you haven't done so
   already.

   * On Windows or MacOS: `Anaconda 2.7 <https://www.continuum.io/downloads>`_.  When installing select "install for all users."  MacOS also offers homebrew (see below).  In our hands, we have had complete success with homebrew, and limited success with anaconda, because of issues with anaconda console script entry points (see below).
...
2. Install libraries that pyspecdata depends on. (If you're interested in why you need to do this first, see installation notes below.)
   ...
   * On Windows: in the Anaconda Prompt, run ``conda install numpy scipy sympy pyqt pytables matplotlib h5py``.

   * For Mac

     * In any case, it's *highly* recommended that you type ``which
       python`` and/or ``which python2``, in case you have several
       versions of python (*e.g.* Anaconda, homebrew, native, *etc.*),
       and need to make sure which one you are invoking,
       as well as ``python -V`` to see what version (2.7, 3.7, etc) the
       python is.
       If your default ``python`` command points to either an anaconda or
       homebrew installation, it makes sense to just use one of those.
       If it points to a system python (``/usr/bin/python``, *etc.*),
       it might be best to install either homebrew or anaconda, since
       (in general) we haven't tested the native python Mac support
       for HDF files, matplotlib, Qt, *etc.*,
       and understand that homebrew and anaconda are better supported.
     * If you have Anaconda, you should be able to run ``conda install
       numpy scipy sympy pyqt pytables matplotlib h5py`` to install
       all the dependencies.
       Then, you can install *via* pip or setup.py, as noted elsewhere
       in this document.
       
       We have noticed some issues with matplotlib when called from a
       script -- these can by solved by creating/editing
       ``~/.matplotlib/matplotlibrc`` and adding the line ``backend: Qt5Agg`` 

       For reasons we don't quite understand, we couldn't get the
       console scripts to work for Anaconda under MacOS
       (this could be specific to particular installations),
       which means that the commands generated by pyspecdata
       (like ``pyspecdata_dataconfig``) don't work properly.
       As a temporary work around, create a script like so to help you
       set up your data configuration (this directly calls the
       console script entry point given in the setup file):

            from pyspecdata.latexscripts import genconfig
            genconfig()
     * You can also use homebrew. *A very important note:* in the
       current version python is renamed to `python2`, and `pip` to
       `pip2`.  Most packages can just be installed with `pip2` under
       homebrew.

       If you want HDF5 functionality, you will need to run ``brew tap
       homebrew/science`` followed by ``brew install hdf5``.
